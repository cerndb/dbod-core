...
sub CreateServer {
	...
	$self->log->info("Parameters ipaddr: <$ipaddr> username: $username vserver: $vserver version: $version password: not displayed");
	...
	if ( scalar (keys(%$nasmounts)) == 1 ) {
		while ( my ($controller, $mountpoint) = each(%$nasmounts) ) {
			...
			if ( $self->IsCmodeMount( $$mountpoint[0]) == 0) {
				$volume_name=&RunTime_Zapi::GetVolumeName7modeFromMount($$mountpoint[0]);
				$iscmode=0;
				chomp $volume_name;
